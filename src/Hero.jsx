...
const SOCIALS = [
  { icon: GithubIcon, href: "https://github.com/TitonouAnaniHounas" },
  { icon: LinkedinIcon, href: "https://www.linkedin.com/in/anani-hounas-titonou-182b88429/" },
  { icon: InstagramIcon, href: "https://instagram.com/hounasx" },
];
...
export default function Hero() {
  ...
  return (
    <section
      id="home"
      className="min-h-screen px-6 lg:px-24 pt-24 lg:pt-28 max-w-[1600px] mx-auto"
    >
      <div className="grid md:grid-cols-2 gap-10 lg:gap-12 items-center w-full">
        {/* Colonne texte */}
        <div>
          <span className="inline-block px-3.5 py-1.5 rounded-full bg-white/5 border border-white/10 text-accent text-sm mb-5">
          ...
          <h1 className="text-6xl md:text-7xl lg:text-8xl font-extrabold leading-[1.05] mb-3">
            <span className="text-text">Developpeur</span>
            <br />
          ...
          <p className="text-2xl lg:text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-primary to-accent mb-2">
            Rôle
          </p>
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {TECHS.map((tech) => (
              <span
                key={tech}
                className="px-3 py-1 rounded-full bg-white/5 border border-white/10 text-sm text-muted"
              >
                {tech}
              </span>
            ))}
          ...
          <div className="flex gap-3 mb-6">
            <a
              href="#portfolio"
              className="flex items-center gap-2 px-6 py-3 rounded-lg bg-gradient-to-r from-primary to-accent text-bg font-semibold text-base hover:opacity-90 transition-opacity"
            >
              Projects <ExternalLink size={16} />
            </a>
            <a
              href="#contact"
              className="flex items-center gap-2 px-6 py-3 rounded-lg border border-white/10 text-text text-base hover:bg-white/5 transition-colors"
            >
            ...
            {SOCIALS.map((social, i) => {
              ...
              return (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-muted hover:text-accent hover:border-accent/40 transition-colors"
                >
                  <SocialIcon size={16} />
                </a>
              );
